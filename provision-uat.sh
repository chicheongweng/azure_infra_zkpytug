...
VNET_ADDRESS_PREFIX=10.101.0.0/20
...
AKS_SUBNET_PREFIX=10.101.0.0/22
AKS_SUBNET_START_IP=10.101.0.1
AKS_SUBNET_END_IP=10.101.3.254
AKS_CLUSTER_ILB_IP=10.101.3.200
SERVICE_CIDR=10.101.16.0/22
DNS_SERVICE_IP=10.101.16.10
MAX_POD_NUMBER=250
K8S_VERSION=1.28
AKS_NODECOUNT=2
AKS_NODECOUNT_MIN=2
...
AKS_SYS_NODE_VM_COMPUTESIZE=Standard_D2s_v5
AKS_USER_NODE_VM_COMPUTESIZE=Standard_D4s_v5
...
APPGW_PREFIX=10.101.5.0/27
APPGW_PRIVATE_IP=10.101.5.10
...
#APP1_INBOUND_SUBNET_PREFIX=10.101.4.32/27
#APP1_OUTBOUND_SUBNET_NAME=snet-zkpytug-rhk-ucp-app-${ENV}-02
#APP1_OUTBOUND_SUBNET_NSG=nsg-zkpytug-rhk-ucp-app-${ENV}-02
#APP1_OUTBOUND_SUBNET_PREFIX=10.101.4.64/27
#APP1_OUTBOUND_SUBNET_START_IP=10.101.4.65
#APP1_OUTBOUND_SUBNET_END_IP=10.101.4.94
...
#APP2_INBOUND_SUBNET_PREFIX=10.101.5.128/27
#APP2_OUTBOUND_SUBNET_NAME=snet-zkpytug-rhk-ucp-app-${ENV}-04
#APP2_OUTBOUND_SUBNET_NSG=nsg-zkpytug-rhk-ucp-app-${ENV}-04
#APP2_OUTBOUND_SUBNET_PREFIX=10.101.5.160/27
#APP2_OUTBOUND_SUBNET_START_IP=10.101.5.161
#APP2_OUTBOUND_SUBNET_END_IP=10.101.5.190
...
FUNC_INBOUND_SUBNET_PREFIX=10.101.4.160/27
FUNC_OUTBOUND_SUBNET_PREFIX=10.101.4.192/27
FUNC_OUTBOUND_SUBNET_START_IP=10.101.4.193
FUNC_OUTBOUND_SUBNET_END_IP=10.101.4.222
...
REDIS_SUBNET_PREFIX=10.101.4.0/27
REDIS_SIZE=C2
REDIS_SKU=Standard
REDIS_REPLICATION=1
...
SQL_SUBNET_PREFIX=10.101.4.32/27
...
STORAGE_SUBNET_PREFIX=10.101.4.64/27
...
ADF_SUBNET_PREFIX=10.101.4.224/27
...
MISC_SUBNET_PREFIX=10.101.5.32/27
...
SVC_KV_NAME=svc-kv-zkpytug-rhk-ucp-dev-01
...
#get object id from service principal name
SVC_KV_ID=$(az ad sp list --display-name $SVC_KV_NAME --query "[0].id" -o tsv)
#assign keyvault secrets access policy to service principal
az keyvault set-policy --name $KV_NAME --resource-group $RESOURCE_GROUP --secret-permissions all --certificate-permissions all --key-permissions all --object-id $SVC_KV_ID
...
# create redis access key secret
...
kubectl create secret docker-registry acr-authentication --docker-server $ACRNAME.azurecr.io  --docker-username $SP_APP_ID --docker-password $SP_APP_SECRET --docker-email chicheong.weng@hkbn.com.hk --namespace ricoh
...
kubectl apply -f spring-boot-server-uat.yaml
...
sleep 30
kubectl apply -f ingress-hello-world-tls-uat.yaml
sleep 30
...
echo "Create Redis"
#redis_id=$(az redis create --name $REDIS_NAME --resource-group $RESOURCE_GROUP --location $REGION --sku $REDIS_SKU --vm-size $REDIS_SIZE --subnet-id $REDIS_SUBNET_ID --query id -o tsv)
...
#echo "Add a redis firewall rule for app service 1"
#az redis firewall-rules create --name $REDIS_NAME --rule-name AllowApp1Subnet --resource-group $RESOURCE_GROUP --start-ip $APP1_OUTBOUND_SUBNET_START_IP --end-ip $APP1_OUTBOUND_SUBNET_END_IP

#echo "Add a redis firewall rule for app service 2"
#az redis firewall-rules create --name $REDIS_NAME --rule-name AllowApp2Subnet --resource-group $RESOURCE_GROUP --start-ip $APP2_OUTBOUND_SUBNET_START_IP --end-ip $APP2_OUTBOUND_SUBNET_END_IP
...
# create redis private end point
...
# Enable publicNetworkAccess to redis private endpoint
...
#set redis access key to key vault ${REDIS_NAME}-accesskey
